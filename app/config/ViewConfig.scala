...
case class ViewConfig(
  ...
  def this(servicesConfig: ServicesConfig, runMode: RunMode) = this(
    ...
    viewVatAccount              = s"""${servicesConfig.baseUrl("vat-summary-frontend")}${servicesConfig.getString("microservice.services.vat-summary-frontend.view-vat-account")}""",
    updateCorrespondenceAddress = s"""${servicesConfig.baseUrl("vat-summary-frontend")}${servicesConfig.getString("microservice.services.vat-summary-frontend.update-correspondence-address")}""",
    feedbackBaseUrl             = servicesConfig.getString("urls.feedback-base"),
    contactBaseUrl              = servicesConfig.getString("urls.contact-frontend"),
    paymentHistoryUrl           = s"""${servicesConfig.baseUrl("vat-summary-frontend")}${servicesConfig.getString("microservice.services.vat-summary-frontend.payments-history")}""",
    btaUrl                      = s"""${servicesConfig.baseUrl("business-tax-account")}${servicesConfig.getString("microservice.services.business-tax-account.bta")}""",
