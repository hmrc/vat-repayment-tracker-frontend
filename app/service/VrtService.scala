    ...
    repaymentDetails match {
      case Some(rd) =>
        ...
        val data = getRepaymentData(rd, financialData)
          def completeExistsWithoutClearingDate(periodKey: String) = data.exists(r => r.periodKey == periodKey && r.riskingStatus.complete && r.clearingDate.isEmpty)
        ...
        val currentData: List[RepaymentDataNoRiskingStatus] = data.filter(
          r => (r.riskingStatus.inProgress && !completeExistsWithoutClearingDate(r.periodKey)) || (r.riskingStatus.complete && r.clearingDate.isEmpty)
