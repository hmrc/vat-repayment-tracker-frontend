    ...
    stubFor(
      get(urlEqualTo(s"""/vat-repayment-tracker-backend/find/vrn/${vrn.value}/${periodKey.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.storedRepaymentDetails1(date, status1, periodKey).toString().stripMargin)
        )
    )

  def repaymentDetailS2(
    vrn:       Vrn,
    date:      String,
    ...
    stubFor(
      get(urlEqualTo(s"""/vat-repayment-tracker-backend/find/vrn/${vrn.value}/${periodKey.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.storedRepaymentDetails2(date, status1, status2).toString().stripMargin)
        )
    )
    ...
    stubFor(
      get(urlEqualTo(s"""/vat-repayment-tracker-backend/find/vrn/${vrn.value}/${periodKey.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.storedRepaymentDetails3(date, status1, status2, status3).toString().stripMargin)
        )
    )
      ...
      get(urlEqualTo(s"""/vat-repayment-tracker-backend/find/vrn/${vrn.value}/${periodKey.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(
              DesData.storedRepaymentDetails3(date1, status1, date2, status2, date3, status3).toString().stripMargin
            )
        )
    )
