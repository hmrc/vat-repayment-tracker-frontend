...
object PaymentsOrchestratorStub extends TestHelper {
  ...
  def ddOk(vrn: Vrn): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/dd-data/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
  ...
  def financialsOkCredit(vrn: Vrn, periodKeys: Seq[String] = Seq("18AG")): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/financial-data/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.financialDataSingleCredit(vrn, periodKeys).toString())
        )
    )
  ...
  def financialsOkDebit(vrn: Vrn, periodKeys: Seq[String] = Seq("18AG")): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/financial-data/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.financialDataSingleDebit(vrn, periodKeys).toString())
        )
    )

  def customerDataOkWithBankDetails(vrn: Vrn, partial: Boolean = false): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/customer-data/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
  ...
  def repaymentDetailsNotFound(vrn: Vrn): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(404)
  ...
  def repaymentDetailS1(
    vrn:         Vrn,
    date:        String,
    status1:     RiskingStatus,
    periodKey:   PeriodKey,
    negativeAmt: Boolean = false
  ): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
    ...
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.repaymentDetails4(date, status1, status2, status3, status4).toString())
        )
    )

  def repaymentDetailSingleCompleted(
    vrn:                    Vrn,
    lastUpdateReceivedDate: LocalDate = LocalDate.of(2001, 1, 1)
  ): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
  ...
  def repaymentDetailsMultipleCompleted(
    vrn:                    Vrn,
    lastUpdateReceivedDate: LocalDate = LocalDate.of(2001, 1, 1)
  ): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.repaymentDetailsMultipleCompleted(lastUpdateReceivedDate).toString())
        )
    )

  def repaymentDetails3Inprogree1Completed(
    vrn:                    Vrn,
    lastUpdateReceivedDate: LocalDate = LocalDate.of(2001, 1, 1)
  ): StubMapping =
    stubFor(
      get(urlEqualTo(s"""/payments-orchestrator/des/repayment-details/vrn/${vrn.value}"""))
        .willReturn(
          aResponse()
            .withStatus(200)
            .withBody(DesData.repaymentDetails3Inprogree1Completed(lastUpdateReceivedDate).toString())
        )
    )
