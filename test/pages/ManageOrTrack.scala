...
object ManageOrTrack extends CommonPage:
  ...
  def assertPageIsDisplayed(
    ...
    if ddDisplayed then dd shouldBe "Manage your Direct Debit"
    if bankDisplayed then bank shouldBe "Manage your repayment bank account"
    if !isShuttered then vrt shouldBe "Track your VAT repayments"
  ...
  def dd(using WebDriver): String = probing(_.findElement(By.id("dd-label")).getText)

  def bank(using WebDriver): String = probing(_.findElement(By.id("bank-label")).getText)

  def vrt(using WebDriver): String = probing(_.findElement(By.id("vrt-label")).getText)

  def clickVrtLabel()(using WebDriver): Unit = probing(_.findElement(By.id("vrt-label")).click())

  def clickBankLabel()(using WebDriver): Unit = probing(_.findElement(By.id("bank-label")).click())
